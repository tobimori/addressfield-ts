...
function unwrapExpressionParentheses(expression: ESTree.Expression) {
  let current = expression;
  while (current.type === "ParenthesizedExpression") current = current.expression;
  return current;
}

function unwrapTypeParentheses(type: ESTree.TSType) {
  let current = type;
...
function isUnknownOrAnyType(type: ESTree.TSType) {
  const unwrapped = unwrapTypeParentheses(type);
  return unwrapped.type === "TSUnknownKeyword" || unwrapped.type === "TSAnyKeyword";
}

function isBroadRecordKeyType(type: ESTree.TSType) {
  const unwrapped = unwrapTypeParentheses(type);
  if (
...
// RETURN TYPE: Mutual recursion with isBroadRecordType needs a stable boolean contract.
...
function isBroadIndexSignature(type: ESTree.TSTypeLiteral) {
...
// RETURN TYPE: Mutual recursion with isBroadRecordReference needs a stable boolean contract.
...
// RETURN TYPE: Callers use this value as the closed BroadTypeKind domain.
...
function assertedExpression(node: ESTree.TSAsExpression | ESTree.TSTypeAssertion) {
  return unwrapExpressionParentheses(node.expression);
}

function assertionFromExpression(expression: ESTree.Expression) {
...
function normalizedTypeText(sourceText: string, type: ESTree.TSType) {
  return sourceText.slice(type.start, type.end).replaceAll(/\s+/gu, "");
}

function typesHaveSameSyntax(sourceText: string, left: ESTree.TSType | null, right: ESTree.TSType) {
  return (
    left !== null &&
...
// RETURN TYPE: Recursive object-type analysis needs a stable boolean contract.
...
// RETURN TYPE: Recursive record-type analysis needs a stable boolean contract.
...
function functionBoundary(node: ESTree.Node) {
...
function variableDeclarator(variable: Variable) {
