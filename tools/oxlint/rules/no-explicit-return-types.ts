...
function hasRequiredReturnTypeComment(sourceCode: SourceCode, node: FunctionImplementation) {
  let current: ESTree.Node = node;
  while (true) {
    if (
      sourceCode
        .getCommentsBefore(current)
        .some((comment) => comment.end <= node.start && /\bRETURN TYPE\s*:/u.test(comment.value))
    ) {
      return true;
    }
    if (current.parent === null || current.parent.type === "Program") return false;
