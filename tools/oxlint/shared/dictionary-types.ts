...
function declaredStatement(statement: ESTree.Statement) {
...
function shadowBuiltIn(name: string, shadowedBuiltIns: Set<string>) {
...
export function createTypeEnvironment(program: ESTree.Program) {
  const aliases = new Map<string, ESTree.TSTypeAliasDeclaration>();
  const interfaces = new Map<string, ESTree.TSInterfaceDeclaration[]>();
  const shadowedBuiltIns = new Set<string>();
  for (const statement of program.body) {
    recordTypeDeclaration(declaredStatement(statement), aliases, interfaces, shadowedBuiltIns);
  }
  return { aliases, interfaces, shadowedBuiltIns };
}

function typeReferenceName(type: ESTree.TSTypeReference) {
  return type.typeName.type === "Identifier" ? type.typeName.name : null;
}

function isBuiltIn(name: string, environment: TypeEnvironment) {
  return BUILT_INS.has(name) && !environment.shadowedBuiltIns.has(name);
}

function isUnappliedReferenceTo(type: ESTree.TSType, name: string) {
...
function unwrapTransparentType(type: ESTree.TSType) {
  let current = type;
  while (
...
function isNeverType(type: ESTree.TSType) {
  return unwrapTransparentType(type).type === "TSNeverKeyword";
}

function isEffectivelyEmptyMember(member: ESTree.TSSignature) {
  return (
...
function isEffectivelyEmptyTypeLiteral(type: ESTree.TSTypeLiteral) {
  return type.members.length === 0 || type.members.every(isEffectivelyEmptyMember);
}

function isEffectivelyEmptyInterface(declarations: readonly ESTree.TSInterfaceDeclaration[]) {
...
// RETURN TYPE: Mutual recursion needs a stable unsafe-value contract.
...
// RETURN TYPE: Mutual recursion needs a stable resolved-type collection contract.
...
export function classifyUnsafeDictionary(type: ESTree.TSType, environment: TypeEnvironment) {
...
// RETURN TYPE: Mutual recursion needs a stable widening-target contract.
...
// RETURN TYPE: Recursive mapped-key analysis needs a stable boolean contract.
...
// RETURN TYPE: Recursive alias analysis needs a stable widening-target contract.
...
export function isKnownEvidenceExpression(expression: ESTree.Expression) {
